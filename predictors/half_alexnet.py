...
class HalfAlexnetFood(nn.Module):
    def __init__(self, name, n_outputs=10, return_feature_domain=False):
        super(HalfAlexnetFood, self).__init__()
        ...
        self.conv5 = nn.Conv2d(96, 64, 3, stride=1, padding=3)
        self.conv5.bias.data.normal_(0, 0.01)
        self.conv5.bias.data.fill_(1.0)  
        ...
        self.batch_norm6 = nn.BatchNorm1d(1024, eps=0.001)
        
        self.fc2 = nn.Linear(1024,1024)
        self.fc2.bias.data.normal_(0, 0.01)
        self.fc2.bias.data.fill_(0) 
        
        self.batch_norm7 = nn.BatchNorm1d(1024, eps=0.001)
...
class HalfAlexnet2(nn.Module):
    def __init__(self, name, n_outputs=10, return_feature_domain=False):
        super(HalfAlexnet2, self).__init__()
    ...
    def forward(self, x):
        ...
        fully1 = self.relu(self.fc1(flatten))
        fully1 = self.batch_norm6(self.drop(fully1))
        fully2_temp = self.fc2(fully1)
        fully2 = self.batch_norm7(self.drop(self.relu(fully2_temp)))
        logits = self.fc3(fully2)
        #softmax_val = self.soft(logits)

        if self.return_feature_domain:
            return logits, fully2_temp
        else:
            return logits
